 ...
 */
int is_column_white(int x, int from, int to);
int is_row_white(int y, int from, int to);
 ...
 */
malloc_arr *nonWhite_columns_ranges();
malloc_arr *nonWhite_rows_ranges();
 ...
 */
square remove_white_lines(square *a);

/* crop_then_saveBMP:
 *      Wants an square for cropping.
 *      After cropping saves that square using saveBMP function.
 */
void crop_then_saveBMP(square *crop, char *output_path);
